...
public class Utils {
	...
	public static String encryptAES128(String value) throws Throwable {
		 ...
		 * AES 암호화
		 */
		SecretKeySpec keySpec 	// AES 암호화 키 생성
				= new SecretKeySpec(CommonProperties.SECURE_KEY.getBytes("UTF-8"), "AES");

		//암호화 객체 생성(AES 방식으로 암호화 작업)
		Cipher cipher = Cipher.getInstance("AES");
		//암호화 모드 설정 및 키 할당
		cipher.init(Cipher.ENCRYPT_MODE, keySpec);				// 초기화
		byte[] encrypted = cipher.doFinal(value.getBytes()); 	// 암호화 결과 (바이트)
		
		/*
		 * Base64 암호화
		 */
		Encoder encoder = Base64.getEncoder();					// Base64 방식의 암호화 객체 생성 (간단한 암호화와 함께 문자열로 바꿔준다)
	...
	public static String decryptAES128(String value) throws Throwable {
		...
		Cipher cipher = Cipher.getInstance("AES");
		//암호화모드 설정 및 키 할당
		cipher.init(Cipher.DECRYPT_MODE, keySpec);	//DECRYPT_MODE : 해제
		/*
		 * Base64 복호화
		 */
		Decoder decoder = Base64.getDecoder();		//Base64 복호화 객체 생성 (string -> byte[])
